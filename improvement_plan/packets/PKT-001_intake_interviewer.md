...
## 목표
프로젝트 시작 직후 (P0 완료, P1 시작 전) 컨설턴트와 구조화된 대화를 통해
보고서 작성에 필요한 핵심 의사결정을 사전에 확정한다.
이 에이전트가 없으면 컨설턴트가 "KSSB로 써줘" 수준의 지시만 할 수밖에 없고,
시스템은 모호한 가정에 기반해 작업하게 된다.

## 입력 파일
- `/Users/lj_homemac/tools/sustainreport_ai/agents/toc-planner.md` (참조용 — 겹치는 역할 확인)
- `/Users/lj_homemac/tools/sustainreport_ai/orchestration/phase_rules.json` (P0 정의 확인)
- `/Users/lj_homemac/tools/sustainreport_ai/CLAUDE.md` (오케스트레이터 헌법 확인)

## 출력 파일
1. **신규 생성**: `/Users/lj_homemac/tools/sustainreport_ai/agents/intake-interviewer.md`
2. **신규 생성**: `/Users/lj_homemac/tools/sustainreport_ai/schemas/intake_manifest.schema.json`

---

## 상세 작업

### STEP 1: `agents/intake-interviewer.md` 생성

아래 내용으로 파일을 생성한다. 각 섹션의 지침을 정확히 따른다.
...
### STEP 2: `schemas/intake_manifest.schema.json` 생성

아래 JSON Schema를 작성한다:

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "intake_manifest.schema.json",
  "title": "Intake Manifest",
  "description": "인테이크 인터뷰 결과 — 보고서 작성 의사결정 사항",
  "type": "object",
  "required": ["manifest_id", "project_id", "interviewed_at", "A_structure", "B_style"],
  "properties": {
    "manifest_id": { "type": "string", "pattern": "^INTAKE-PRJ-" },
    "project_id": { "type": "string", "pattern": "^PRJ-" },
    "interviewed_at": { "type": "string", "format": "date-time" },
    "agent": { "type": "string" },
    "A_structure": {
      "type": "object",
      "required": ["toc_style", "scope_inclusions", "scope_exclusions"],
      "properties": {
        "toc_style": { "type": "string", "enum": ["framework", "theme", "custom"] },
        "custom_toc_reference": { "type": ["string", "null"] },
        "scope_inclusions": { "type": "array", "items": { "type": "string" } },
        "scope_exclusions": { "type": "array", "items": { "type": "string" } },
        "section_priority": { "type": "array", "items": { "type": "string" } },
        "section_priority_rationale": { "type": "string" }
      }
    },
    "B_style": {
      "type": "object",
      "required": ["narrative_style", "sentence_ending"],
      "properties": {
        "narrative_style": { "type": "string", "enum": ["framework", "activity", "mixed"] },
        "mixed_section_map": { "type": "object" },
        "sentence_ending": { "type": "string" },
        "company_name_rule": { "type": "string" },
        "english_parallel": { "type": "boolean" }
      }
    },
    "C_references": {
      "type": "object",
      "properties": {
        "previous_report_path": { "type": ["string", "null"] },
        "benchmark_report_paths": { "type": "array" },
        "materiality_assessment_available": { "type": "boolean" },
        "high_materiality_topics": { "type": "array", "items": { "type": "string" } }
      }
    },
    "D_data": {
      "type": "object",
      "properties": {
        "quantitative_data_status": { "type": "string", "enum": ["none", "partial", "most"] },
        "confirmed_data_files": { "type": "array" },
        "must_include_achievements": { "type": "array", "items": { "type": "string" } }
      }
    },
    "open_notes": { "type": "string" }
  }
}
```
...
다음 세 가지가 모두 충족되어야 PKT-001을 done 처리한다:

1. `agents/intake-interviewer.md` 파일이 존재하고, 상기 내용을 포함한다
2. `schemas/intake_manifest.schema.json` 파일이 존재하고 유효한 JSON Schema이다
3. 기존 `agents/toc-planner.md` 의 **역할 섹션에** 다음 문장이 추가됐는지 확인 후, 없으면 추가한다:
   > `intake-interviewer` 에이전트가 생성한 `00_definition/intake_manifest.json`을 반드시 먼저 읽은 뒤 목차 설계를 시작한다. 이 파일이 없으면 작업을 시작하지 않고 오케스트레이터에게 인테이크 인터뷰 미완료를 보고한다.

---

## 결정 원칙

- `agents/intake-interviewer.md` 작성 시 마크다운 코드블록 안의 내용은 그대로 복사한다
- JSON Schema는 표준 draft-2020-12를 따른다
- 기존 `toc-planner.md` 수정 시 반드시 `toc-planner.md.bak`을 먼저 생성한다
